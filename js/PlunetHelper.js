...
chrome.storage.local.get(null, function (settings) {
    ...
    for (var i = 0; i < tags.length; i++) {
        ...
        if (cleanUp) { // get rid of unnecessary linebreaks
            ...
            if (tags[i].textContent.match(/(<div>)(<br\s*\/>)(<span[^>]*>.*?<\/span><\/div>\s*)/gi) !== null) {
                tags[i].textContent = tags[i].textContent.replace(/(<div>)(<br\s*\/>)(<span[^>]*>.*?<\/span><\/div>\s*)/gi, '$1$3');
            }
            if (tags[i].textContent.match(/(\/span><\/strong><br\/>)(<br\/>)(\s*<\/span>\s*<\/div>\s*<div>\s*<p>\s*<span)/gi) !== null) {
                tags[i].textContent = tags[i].textContent.replace(/(\/span><\/strong><br\/>)(<br\/>)(\s*<\/span>\s*<\/div>\s*<div>\s*<p>\s*<span)/gi, '$1$3');
            }
        }
        if (makeBold) { // look for *bold* shortcuts and format as <strong>bold</strong>
            if (tags[i].textContent.match(/\*+([^<>]+?)\*+/gim) !== null) {
                tags[i].textContent = tags[i].textContent.replace(/\*+([^<>]+?)\*+/gim, '<strong>$1</strong>');
            }
        }
        if (makeImportant) { // look for §important§ shortcuts and format as <strong style="color:red;">important</strong>
            if (tags[i].textContent.match(/\§+([^<>]+?)\§+/gim) !== null) {
                tags[i].textContent = tags[i].textContent.replace(/\§+([^<>]+?)\§+/gim, '<strong><span style="color:#ff0000;">$1</span></strong>');
            }
        }
        if (makeHighlighted) { // look for #highlighting# shortcuts and format accordingly
            if (tags[i].textContent.match(/\~+([^<>]+?)\~+/gim) !== null) {
                tags[i].textContent = tags[i].textContent.replace(/\~+([^<>]+?)\~+/gim, '<span style="color:' + highColor + ';background-color:' + highBColor + ';">$1</span>');
            }
        }
